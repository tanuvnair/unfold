...
help: ## Show available targets
	...
	@printf '\nDefaults: BIN=$(BIN)  API_BIN=$(API_BIN)  UNFOLD_API_CONFIG=$(UNFOLD_API_CONFIG)\n'
...
cli: ## Build the CLI binary → ./unfold
...
sync-ui: web-build ## Copy web/dist into the embed tree for the API binary
	rm -rf $(WEBUI_DIST)
	mkdir -p $(WEBUI_DIST)
	cp -a $(WEB_DIR)/dist/. $(WEBUI_DIST)/

api-build: sync-ui ## Build API+UI binary → ./unfold-api
	$(GO) build -o $(API_BIN) ./cmd/api

serve: api-build ## Build and run the single-binary UI+API
	./$(API_BIN) -config $(UNFOLD_API_CONFIG)
