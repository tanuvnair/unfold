# unfold — local CLI, API, and web UI
#
# Usage:  make <target>
# Config: copy .env.example → .env (optional); make includes it when present.
# Go module lives in $(GO_DIR); all Go targets use `go -C $(GO_DIR)` so
# configs/ paths resolve relative to $(GO_DIR), not repo root.

.DEFAULT_GOAL := help

DIST        ?= dist
CLI_BIN     ?= unfold-cli
API_BIN     ?= unfold-api
GO          ?= go
GO_DIR      ?= core
WEB_DIR     ?= web
NPM         ?= npm
WEB_DIST    := $(WEB_DIR)/dist
WEBUI_DIST  := $(GO_DIR)/internal/webui/dist

# Absolute, so builds are correct regardless of GO_DIR's nesting depth.
DIST_ABS    := $(CURDIR)/$(DIST)

# Defaults; .env values (when present) override via include/export below.
# Resolved relative to $(GO_DIR) when launching api/serve.
UNFOLD_API_CONFIG ?= configs/banks.json

# Load repo-root .env into the environment when present.
ifneq (,$(wildcard .env))
  include .env
  export
endif

.PHONY: help \
	cli-build api api-build \
	web web-install web-build sync-ui \
	dev \
	test fmt vet tidy check \
	build serve clean

help: ## Show available targets
	@printf 'unfold make targets\n\n'
	@awk 'BEGIN {FS = ":.*## "}; /^[a-zA-Z0-9_-]+:.*?## / {printf "  %-14s %s\n", $$1, $$2}' $(MAKEFILE_LIST)
	@printf '\nDefaults: DIST=$(DIST)  CLI_BIN=$(CLI_BIN)  API_BIN=$(API_BIN)  GO_DIR=$(GO_DIR)  UNFOLD_API_CONFIG=$(UNFOLD_API_CONFIG)\n'
	@printf 'Env:      UNFOLD_API_HOST UNFOLD_API_PORT UNFOLD_API_CONFIG UNFOLD_API_CORS_ORIGIN\n'
	@printf '          UNFOLD_WEB_HOST UNFOLD_WEB_PORT UNFOLD_WEB_API_BASE UNFOLD_API_PROXY_TARGET\n'

##@ Go

cli-build: ## Build the CLI binary → ./dist/unfold-cli
	mkdir -p $(DIST)
	$(GO) -C $(GO_DIR) build -o $(DIST_ABS)/$(CLI_BIN) ./cmd/cli

api: ## Run the HTTP API from source (loads UNFOLD_* from .env; CWD=$(GO_DIR))
	$(GO) -C $(GO_DIR) run ./cmd/api -config $(UNFOLD_API_CONFIG)

sync-ui: web-build ## Copy web/dist into the embed tree for the API binary
	rm -rf $(WEBUI_DIST)
	mkdir -p $(WEBUI_DIST)
	cp -a $(WEB_DIST)/. $(WEBUI_DIST)/

api-build: sync-ui ## Build API+UI binary → ./dist/unfold-api
	mkdir -p $(DIST)
	$(GO) -C $(GO_DIR) build -o $(DIST_ABS)/$(API_BIN) ./cmd/api

serve: api-build ## Build and run the single-binary UI+API (CWD=$(GO_DIR))
	cd $(GO_DIR) && $(DIST_ABS)/$(API_BIN) -config $(UNFOLD_API_CONFIG)

##@ Web

web-install: ## Install web npm dependencies
	$(NPM) --prefix $(WEB_DIR) install

web: ## Run the Vite web UI (proxies /api to the Go API)
	$(NPM) --prefix $(WEB_DIR) run dev

web-build: ## Build the web UI for production → ./web/dist
	$(NPM) --prefix $(WEB_DIR) run build

##@ Dev flow

dev: ## Run API (from source) and web dev server together; Ctrl+C stops both
	@trap 'kill 0' EXIT INT TERM; \
	$(MAKE) api & \
	$(MAKE) web & \
	wait

##@ Quality

test: ## Run Go tests
	$(GO) -C $(GO_DIR) test ./...

fmt: ## Format Go sources
	$(GO) -C $(GO_DIR) fmt ./...

vet: ## Run go vet
	$(GO) -C $(GO_DIR) vet ./...

tidy: ## Tidy go.mod / go.sum
	$(GO) -C $(GO_DIR) mod tidy

check: fmt vet test ## Format, vet, and test

##@ Meta

build: cli-build api-build ## Build CLI and API+UI binaries

clean: ## Remove built binaries and restore placeholder embed dist
	rm -rf $(DIST) $(WEB_DIST) $(WEBUI_DIST)
	mkdir -p $(WEBUI_DIST)
	touch $(WEBUI_DIST)/.gitkeep
